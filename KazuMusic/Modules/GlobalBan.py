...
from KazuMusic import BOT_ID, BOT_NAME, SUDO_USERS, app
from KazuMusic.Helpers.Database import (add_gban_user, get_served_chats, is_gbanned_user, remove_gban_user)
...
@app.on_message(filters.command("mgban") & filters.user(SUDO_USERS))
async def ban_globally(_, message):
    if not message.reply_to_message:
        if len(message.command) < 2:
            await message.reply_text("**ᴇxᴀᴍᴩʟᴇ :**\n/mgban [ᴜsᴇʀɴᴀᴍᴇ|ᴜsᴇʀ ɪᴅ]")
            return
...
**• ᴜsᴇʀ :** {user.mention}
**• ᴜsᴇʀ ɪᴅ:** `{user.id}`
**• ʙᴀɴɴᴇᴅ ɪɴ :** {number_of_chats} ɢʀᴏᴜᴘ"""
            try:
                await m.delete()
            except Exception:
                pass
...
@app.on_message(filters.command("mungban") & filters.user(SUDO_USERS))
